...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _autoProcessMonthlySalaries(); // <--- Auto-salary generator call
    _loadKpiData();
    _loadStockData();
  ...
  Future<void> _autoProcessMonthlySalaries() async {
    ...
    // 1. Sabse pehle saare Monthly workers ka latest record nikalo
    for (var entry in history.reversed) {
      // reversed taaki sabse naya record last mein overwrite ho
    ...
    // 2. Har worker ke liye check karo ki is mahine ki salary add hui hai ya nahi
    for (var workerName in activeWorkers.keys) {
      var workerData = activeWorkers[workerName]!;

      // Agar worker inactive (chhod chuka) hai, toh use chhod do
      if (workerData['isActive'] == false) continue;

      // Check karo kya CURRENT mahine mein is naam ka expense pehle se hai
      ...
      if (!alreadyAdded) {
        // Agar nahi hai, toh auto-generate karo
        ...
        autoEntry['isAutoGenerated'] =
            true; // Taaki pata rahe ki ye system ne kiya hai
        history.insert(0, autoEntry);
        dataChanged = true;
      }
    }

    // 3. Agar koi nayi salary add hui hai, toh database update karo
    if (dataChanged) {
