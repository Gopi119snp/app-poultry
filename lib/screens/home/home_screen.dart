...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  Future<void> _autoProcessMonthlySalaries() async {
    ...
    Map<String, Map<String, dynamic>> activeWorkers = {};
    final now = DateTime.now();
    final currentMonthKey =
        "${now.year}-${now.month.toString().padLeft(2, '0')}";

    for (var entry in history.reversed) {
      if (entry['unitMode'] == 'Monthly') {
        activeWorkers[entry['workerName']] = Map<String, dynamic>.from(entry);
      }
    }

    bool dataChanged = false;

    for (var workerName in activeWorkers.keys) {
      var workerData = activeWorkers[workerName]!;
      if (workerData['isActive'] == false) continue;

      bool alreadyAdded = history.any((e) {
        if (e['workerName'] != workerName) return false;
        if (e['unitMode'] != 'Monthly') return false;
        DateTime? d = DateTime.tryParse(e['date'].toString());
        return d != null && d.year == now.year && d.month == now.month;
      });

      if (!alreadyAdded) {
        Map<String, dynamic> autoEntry = Map<String, dynamic>.from(workerData);
        autoEntry['date'] = DateTime.now().toIso8601String();
        autoEntry['note'] = "🤖 Auto-generated salary for $currentMonthKey";
        autoEntry['isAutoGenerated'] = true;
        history.insert(0, autoEntry);
        dataChanged = true;
      }
    }
  ...
  Future<void> _loadStockData() async {
    ...
    final loadedMedicine = loadedMedicineRaw
    ...
    for (final med in loadedMedicine) {
      ...
      remainingMap[mId] = await computeMedicineRemainingBaseQty(med);
    }

    if (!mounted) return;
    setState(() {
      _feedStockList = loadedFeed;
              ...
              try {
                if (docType == 'Aadhaar') {
                  ...
                  if (frontPath != null) {
                    ...
                    if (backPath != null) {
                      setModalState(() {
                        aadhaarPath = "$frontPath|$backPath";
                      });
                      Get.snackbar(
                        'Success',
  ...
  Widget _buildFeedStockTab() {
    ...
    final sorted = List<Map<String, dynamic>>.from(_feedStockList)
      ..sort(
        (a, b) => kFeedTypeIds
            .indexOf(a['id'])
        ...
        children: [
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                final feedType = sorted[index];
                final String id = feedType['id']?.toString() ?? '';
                final double qty = computeFeedRemaining(feedType);
                bool isWhole = qty == qty.roundToDouble();
                MaterialColor c = colors[id] ?? Colors.green;
                bool isOut = qty <= 0;
  ...
  Widget _medicineStockCard(Map<String, dynamic> medicine) {
    ...
    final double weightedAvgCost =
        (medicine['weightedAvgCost'] as num?)?.toDouble() ?? 0.0;
    final double currentFarmerRate =
        (medicine['currentFarmerRate'] as num?)?.toDouble() ?? 0.0;
    final double remainingQuantity = _medicineRemainingBase[mId] ?? 0.0;
    final double remainingValue = remainingQuantity * weightedAvgCost;
    final bool isOut = remainingQuantity <= 0.0001;
          ...
          Row(
            children: [
              ...
              Expanded(
                child: _stockMiniStat(
                  'Avg Cost',
                  '₹${weightedAvgCost.toStringAsFixed(2)}/$unit',
                ),
              ),
            ],
          ),
...
// ── ✅ SMARTSCANNER CLASSES (unchanged) ──
